...
class fleiss_kappa:
    ...
    def calc_std_dev(self):
	        #From https://i1.wp.com/www.real-statistics.com/wp-content/uploads
            #/2013/11/image102c.png	
...
for title in titles:
    ...
    c = conn.cursor()
    c.execute('select thread_id from post2 inner join thread on post2.thread_id=\
            thread.id where original=1 and post2.courseid like "%%warhol%%" and \
            thread.title like '+'"%%'+title+'%%"') 
    thread_id = c.fetchone()
    c.execute("select count(1) from post2 where thread_id=? and courseid like \
            '%warhol%'",
            thread_id)
    post2 = c.fetchone()
    c.execute("select count(1) from comment2 where thread_id=? and courseid like  \
            '%warhol%'", thread_id)
    comment2 = c.fetchone()
