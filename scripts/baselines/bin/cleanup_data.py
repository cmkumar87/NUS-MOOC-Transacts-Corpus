...
courses_df = pd.read_csv('../data/organalysis-003_out_.csv', encoding = "utf8")
...
for thread_title in thread_titles:
    ...
    courseid = courses_df.loc[courses_df['Title']==thread_title,'Course'].iloc[0].strip()
    thread_text = ''
    thread_comment_text = ''
    post_count = 0
    comment_count = 0
    for row in c.execute('select thread.id, post2.id, post2.post_text from post2 inner join thread on \
        post2.thread_id=thread.id where trim(thread.title)=? and post2.courseid=?  order by post2.post_order ASC',
        (thread_title,courseid)):
        post_count+=1
...
courses_df.to_csv('../data/preprocessed.csv')
